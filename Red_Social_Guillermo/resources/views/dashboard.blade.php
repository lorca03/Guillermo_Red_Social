        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-4 flex items-center justify-center">
            <div class="bg-white overflow-hidden w-96 shadow-xl shadow-indigo-500/40 sm:rounded-lg p-4 flex flex-col items-center justify-center">
                @foreach($images as $image)
                    ...
                    <labe>Commentarios --> {{count($image->comments)}} <button type="submit">Ver todos</button> </labe>
                    <br>
                    <form method="POST" action="{{ route('save.comentario') }}">
                        @csrf
                        ...
                        <input class="bg-indigo-500 rounded-lg p-2 text-white" type="submit" value="Comentar">
                    </form>
                    <br>
                @endforeach
{{--                {{$images->links()}}--}}
